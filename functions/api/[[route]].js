...
async function ensureTables(db) {
  await db.prepare(`CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, val TEXT)`).run();

  await db.prepare(`
    CREATE TABLE IF NOT EXISTS users (
      id TEXT PRIMARY KEY, userno TEXT UNIQUE, eml TEXT UNIQUE,
      pwd TEXT, nam TEXT, rol TEXT DEFAULT 'user',
      pln TEXT DEFAULT 'free', pln_exp INTEGER DEFAULT 0,
      sts INTEGER DEFAULT 1, ip_hash TEXT, cat INTEGER
    )
  `).run();
  ...
  await db.prepare(`
    CREATE TABLE IF NOT EXISTS polls (
      id TEXT PRIMARY KEY, uid TEXT, tit TEXT, slg TEXT,
      sts INTEGER DEFAULT 1, typ INTEGER DEFAULT 0,
      clr TEXT DEFAULT '#1a73e8', pwd TEXT, exp INTEGER,
      opt TEXT, snap TEXT, rpt TEXT,
      ended INTEGER, cleaned INTEGER, cat INTEGER
    )
  `).run();
  ...
  await db.prepare(`
    CREATE TABLE IF NOT EXISTS votes (
      id TEXT PRIMARY KEY, pid TEXT, oid TEXT,
      fp TEXT, ip TEXT, cat INTEGER
    )
  `).run();
  ...
  await db.prepare(`
    CREATE TABLE IF NOT EXISTS admin_keys (
      ...
      salt TEXT, test TEXT, updated INTEGER
    )
  `).run();
...
async function handleAuth(method, path, body, db, request) {
  if (method === 'POST' && path === '/auth/signup') {
    ...
    await db.prepare('INSERT INTO users (id, userno, eml, pwd, nam, rol, pln, sts, ip_hash, cat) VALUES (?,?,?,?,?,?,?,?,?,?)')
  ...
  if (method === 'POST' && path === '/auth/login') {
    ...
    const hashedPwd = await sha256(password);
    const user = await db.prepare('SELECT id, userno, eml, nam, rol, pln, pln_exp, sts FROM users WHERE eml = ? AND pwd = ?')
...
async function handlePolls(method, path, body, db, request) {
  ...
  if (method === 'GET' && path === '/polls') {
    const { results } = await db.prepare('SELECT id, tit, slg, sts, typ, clr, exp, opt, snap, cat, ended FROM polls WHERE uid = ? ORDER BY cat DESC LIMIT 50')
      .bind(user.id).all();
  ...
  if (method === 'POST' && path === '/polls') {
    ...
    await db.prepare('INSERT INTO polls (id, uid, tit, slg, sts, typ, clr, pwd, exp, opt, cat) VALUES (?,?,?,?,?,?,?,?,?,?,?)')
  ...
  if (method === 'GET' && pollMatch) {
    ...
    const results = opts.map(o => ({ id: o.id, text: o.text, count: (vr.find(v => v.oid === o.id) || {}).count || 0, percent: total > 0 ? Math.round((((vr.find(v => v.oid === o.id) || {}).count || 0) / total) * 100) : 0 }));
  ...
  if (method === 'PUT' && pollMatch) {
    const poll = await db.prepare('SELECT * FROM polls WHERE id = ? AND uid = ?').bind(pollMatch[1], user.id).first();
    if (!poll) return err('Poll not found', 404);
    if (poll.sts !== 1) return err('Cannot edit', 403);
  ...
  if (method === 'POST' && closeMatch) {
    ...
    if (!poll) return err('Poll not found', 404);
    if (poll.sts !== 1) return err('Already closed', 403);
    const { results: vr } = await db.prepare('SELECT oid, COUNT(*) as count FROM votes WHERE pid = ? GROUP BY oid').bind(poll.id).all();
    const opts = JSON.parse(poll.opt || '[]');
    const results = opts.map(o => ({ id: o.id, text: o.text, count: (vr.find(v => v.oid === o.id) || {}).count || 0 }));
    const snap = JSON.stringify(results);
    await db.prepare('UPDATE polls SET sts = 2, snap = ?, ended = ?, cleaned = ? WHERE id = ?').bind(snap, Date.now(), Date.now(), poll.id).run();
...
async function handlePublic(method, path, body, db, request) {
  ...
  if (method === 'POST' && voteMatch) {
    ...
    if (!option || !fingerprint) return err('Missing fields', 400);
    ...
    if (poll.exp && Date.now() > poll.exp) return err('Poll ended', 410);
  ...
  if (method === 'POST' && reportMatch) {
    ...
    let reports = [];
    if (poll.rpt) {
      try { reports = JSON.parse(poll.rpt); } catch { reports = []; }
    }

    const idx = reports.findIndex(r => r.fp === fingerprint);
    if (idx > -1) {
      reports.splice(idx, 1);
    } else {
      reports.push({ fp: fingerprint, ts: Date.now() });
    }

    await db.prepare('UPDATE polls SET rpt = ? WHERE id = ?').bind(JSON.stringify(reports), poll.id).run();
    return json({ success: true, reported: idx === -1, reportCount: reports.length });
  }

  // ─── REPORT STATUS: Check if user reported ───────────────
  const reportStatusMatch = path.match(/^\/public\/poll\/([a-zA-Z0-9-]+)\/report-status$/);
  if (method === 'POST' && reportStatusMatch) {
    const { fingerprint } = body;
    if (!fingerprint) return err('Fingerprint required', 400);

    const poll = await db.prepare('SELECT rpt FROM polls WHERE slg = ?').bind(reportStatusMatch[1]).first();
    if (!poll) return err('Poll not found', 404);
    ...
    return json({ reported: reports.some(r => r.fp === fingerprint), reportCount: reports.length });
  }
...
async function handleAdmin(method, path, body, db, request) {
  const publicPaths = ['/admin/check-passphrase', '/admin/setup-passphrase', '/admin/verify-passphrase'];
  ...
  if (method === 'GET' && path === '/admin/check-passphrase') {
    ...
    return json({ exists: !!(ak && ak.test), salt: ak ? ak.salt : null });
  }
  ...
  if (method === 'POST' && path === '/admin/setup-passphrase') {
    ...
    if (existing && existing.test) return err('Passphrase already set', 409);
    const { salt, testEncrypted } = body;
    await db.prepare('INSERT OR REPLACE INTO admin_keys (id, salt, test, updated) VALUES (?,?,?,?)').bind('admin', salt, testEncrypted, Date.now()).run();
    return json({ success: true });
  }
  ...
  if (method === 'GET' && path === '/admin/stats') {
    ...
    return json({ users: uc.c, polls: pc.c, activePolls: ac.c, votes: vc.c, freeUsers: fu.c, proUsers: pu.c, maxUsers: mu.c, reportedPolls: rp.c, pollingEnabled: pe && pe.val === '1' });
  }
  ...
  if (method === 'POST' && blockMatch) { await db.prepare('UPDATE users SET sts = 0 WHERE id = ?').bind(blockMatch[1]).run(); return json({ success: true }); }

  const unblockMatch = path.match(/^\/admin\/user\/([a-zA-Z0-9-]+)\/unblock$/);
  if (method === 'POST' && unblockMatch) { await db.prepare('UPDATE users SET sts = 1 WHERE id = ?').bind(unblockMatch[1]).run(); return json({ success: true }); }
  ...
  if (method === 'DELETE' && delUserMatch) {
    ...
    for (const p of polls.results) { await db.prepare('DELETE FROM votes WHERE pid = ?').bind(p.id).run(); }
  ...
  if (method === 'POST' && adminBlockP) { await db.prepare('UPDATE polls SET sts = 0 WHERE id = ?').bind(adminBlockP[1]).run(); return json({ success: true }); }

  const adminUnblockP = path.match(/^\/admin\/poll\/([a-zA-Z0-9-]+)\/unblock$/);
  if (method === 'POST' && adminUnblockP) { await db.prepare('UPDATE polls SET sts = 1 WHERE id = ?').bind(adminUnblockP[1]).run(); return json({ success: true }); }

  // ─── Clear reports for a poll ────────────────────────────
  const clearReportsMatch = path.match(/^\/admin\/poll\/([a-zA-Z0-9-]+)\/clear-reports$/);
  if (method === 'POST' && clearReportsMatch) {
    await db.prepare('UPDATE polls SET rpt = ? WHERE id = ?').bind('[]', clearReportsMatch[1]).run();
    return json({ success: true });
  }
  ...
  if (method === 'GET' && path === '/admin/storage') {
    ...
    return json({ stats, totalSize: fmt(total), totalBytes: total, limitBytes: 5 * 1024 * 1024 * 1024, percentUsed: ((total / (5 * 1024 * 1024 * 1024)) * 100).toFixed(4), endedPolls: (await db.prepare("SELECT COUNT(*) as c FROM polls WHERE sts = 2 AND cleaned > 0").first()).c });
  }

  if (method === 'POST' && path === '/admin/cleanup') {
    const r = await db.prepare('DELETE FROM polls WHERE sts = 2 AND cleaned < ? AND cleaned > 0').bind(Date.now() - (7 * 24 * 60 * 60 * 1000)).run();
...
export async function onRequest(context) {
  ...
  try {
    await db.prepare('DELETE FROM polls WHERE sts = 2 AND cleaned < ? AND cleaned > 0').bind(Date.now() - (7 * 24 * 60 * 60 * 1000)).run();
  } catch {}
